<template>
  <div class="container">
    ...
    <form class="form">
      <div class="prod1">
        <label for="produto1">Produto do eixo X:</label>
        <label for="produto1">Nome do Produto</label>
        <input type="text" id="produto1" v-model="produto1" />
        <label for="x1"> Semana 1 - {{ this.produto1 }}  </label>
        <input type="number" id="x1" v-model="x1" />
        <label for="x2"> Semana 2 - {{ this.produto1 }}  </label>
        <input type="number" id="x2" v-model="x2" />
        <label for="x3"> Semana 3 - {{ this.produto1 }}  </label>
        <input type="number" id="x3" v-model="x3" />
        <label for="x4"> Semana 4 - {{ this.produto1 }}  </label>
        <input type="number" id="x4" v-model="x4" />
      </div>
      <div class="prod2">
        <label for="produto2">Produto do eixo Y:</label>
        <label for="produto2">Nome do Produto</label>
        <input type="text" id="produto2" v-model="produto2" />
        <label for="y1">Semana 1 - {{ this.produto2 }}</label>
        <input type="number" id="y1" v-model="y1" />
        <label for="y2">Semana 2 - {{ this.produto2 }}</label>
        <input type="number" id="y2" v-model="y2" />
        <label for="y3">Semana 3 - {{ this.produto2 }}</label>
        <input type="number" id="y3" v-model="y3" />
        <label for="y4">Semana 4 - {{ this.produto2 }}</label>
...
setup() {
 ...
 const lineOptions = ref({
  title: {
    text: 'Cruva de Possibilidade de Produção - Mês (4 semanas)',
    left:  'center',
    textStyle: {
      fontSize: 25,
      color: '#fff',
  ...
  series: [{
    data: [],
    type: 'line',
    name: 'Curva de Possibilidade de Produção',
    smooth: true,
    symbol: 'circle',
    symbolSize: 10,
    itemStyle: {
...
methods: {
  hanldeGrafic () {
    this.x5 = this.x1 + this.x2 + this.x3 + this.x4
    this.y5 = this.y1 + this.y2 + this.y3 + this.y4
    this.lineOptions.xAxis.data.push(this.x0, this.x1, this.x2, this.x3, this.x4)
    this.lineOptions.series[0].data.push(this.y4, this.y3, this.y2, this.y1, this.y0)
